	...
	while (true) { 
		...
		if (command == "open" || command == "o") {
			...
			try {
				...
				catch (vector<pair<int, const char *>> error_list) {
					sort(error_list.begin(), error_list.end());
					for (auto error: error_list)
						cout << "Compile error: " << error.second << " in line: " << error.first << '\n';
					cout << "Ignore strings with compile errors and build ini-file no matter what? y/n\n";
					char ch = 'n';
					cin >> ch;
					if (ch == 'y') {
						ip.compile(true);
						cout << "File was compiled. Ignored strings:\n";
						for (auto error : error_list)
							cout << error.first << ": " << data[error.first] << '\n';
					}
					else
						cout << "Try open another ini-file\n";
				}
			}
			catch (pair<int, const char*> exeption) {
				cout << "Runtime error: " << exeption.second << '\n';
			}
			continue;
		}
